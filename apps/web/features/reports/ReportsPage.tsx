...
export default function ReportsPage() {
  ...
  const handleGenerateComplianceReport = async (format: 'pdf' | 'json' = 'pdf') => {
    ...
    try {
      if (format === 'pdf') {
        const response = await fetch(`/api/reports/compliance?format=pdf&charts=true`, {
          credentials: 'include',
        });
  ...
  const handleGenerateExecutiveSummary = async () => {
    ...
    setError('');
    try {
      const response = await fetch(`/api/reports/executive-summary?format=pdf&charts=true`, {
        credentials: 'include',
      });
        ...
        <div className="bg-white rounded-lg shadow p-8">
          ...
          <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3">
            <div className="border border-gray-200 rounded-lg p-6 hover:shadow-md transition-shadow">
              ...
              <p className="text-gray-600 text-sm mb-4">Comprehensive risk analysis with trend analysis and gap identification.</p>
              <div className="flex gap-2">
                <button
                  onClick={() => void handleGenerateRiskReport('pdf')}
                  disabled={loading}
                ...
                <button
                  onClick={() => void handleGenerateRiskReport('json')}
                  disabled={loading}
            ...
            <div className="border border-gray-200 rounded-lg p-6 hover:shadow-md transition-shadow">
              ...
              <p className="text-gray-600 text-sm mb-4">Compliance status with gap analysis across GDPR, NIS2, and other frameworks.</p>
              <div className="flex gap-2">
                <button
                  onClick={() => void handleGenerateComplianceReport('pdf')}
                  disabled={loading}
                  className="flex-1 px-4 py-2 bg-blue-600 text-white rounded-md hover:bg-blue-700 text-sm font-medium disabled:opacity-50"
                >
                  {loading ? 'Generating...' : 'PDF'}
                ...
                <button
                  onClick={() => void handleGenerateComplianceReport('json')}
                  disabled={loading}
                  className="flex-1 px-4 py-2 bg-gray-600 text-white rounded-md hover:bg-gray-700 text-sm font-medium disabled:opacity-50"
                >
                  JSON
            ...
            <div className="border border-gray-200 rounded-lg p-6 hover:shadow-md transition-shadow">
              ...
              <p className="text-gray-600 text-sm mb-4">High-level overview with key metrics, trends, and recommendations.</p>
              <button
                onClick={() => void handleGenerateExecutiveSummary()}
                disabled={loading}
            ...
            <div className="border border-gray-200 rounded-lg p-6 hover:shadow-md transition-shadow">
              ...
              <p className="text-gray-600 text-sm mb-4">Evidence, controls, snapshots, and recent audit log as one JSON bundle for auditors.</p>
              <button
                onClick={() => void handleDownloadAuditPackage()}
                disabled={loading}
            ...
            <div className="border border-gray-200 rounded-lg p-6 hover:shadow-md transition-shadow">
              ...
              <p className="text-gray-600 text-sm mb-4">Unified export containing the audit package, governance manifest, and optional attachments.</p>
              <button
                onClick={() => void handleDownloadAuditorZip()}
                disabled={loading}
            ...
            <div className="border border-gray-200 rounded-lg p-6 hover:shadow-md transition-shadow">
              ...
              <p className="text-gray-600 text-sm mb-4">Build a custom report with selected sections and filters.</p>
              <button
            ...
            <div className="border border-gray-200 rounded-lg p-6 hover:shadow-md transition-shadow">
              ...
              <p className="text-gray-600 text-sm mb-4">Export your complete AI tool inventory as CSV or Excel.</p>
              <div className="flex gap-2">
                <button
                  onClick={() => void handleExportInventory('csv')}
                  disabled={loading}
                ...
                <button
                  onClick={() => void handleExportInventory('excel')}
                  disabled={loading}
                  className="flex-1 px-4 py-2 bg-blue-600 text-white rounded-md hover:bg-blue-700 text-sm font-medium disabled:opacity-50"
                >
                  Export Excel
            ...
            <CustomReportBuilder
              ...
              onGenerate={async (options) => {
                ...
                try {
                  ...
                  const response = await fetch(`/api/reports/custom?format=pdf`, {
                    method: 'POST',
                    headers,
                    credentials: 'include',
...
function CustomReportBuilder({
  ...
  const toggleSection = (sectionId: string) => {
    setSections((prev) => (prev.includes(sectionId) ? prev.filter((id) => id !== sectionId) : [...prev, sectionId]));
  };
