  ...
  return (
    <AppLayout>
                    ...
                    .map((decision) => (
                      <div key={decision.id} className="border-l-4 border-blue-500 pl-4 py-2">
                        <div className="flex items-center justify-between">
                          ...
                          <span className="text-xs text-gray-500">{new Date(decision.createdAt).toLocaleString()}</span>
                        </div>
                        {decision.rationale && <p className="mt-2 text-sm text-gray-700">{decision.rationale}</p>}
                        {decision.approvedBy && (
                          <p className="mt-1 text-xs text-gray-500">Approved by: {decision.approvedBy}</p>
                        )}
                      </div>
                    ))}
                </div>
            ...
            <div className="bg-white shadow rounded-lg p-6">
              ...
              <dl className="space-y-3">
                <div>
                  ...
                  <dd className="text-sm text-gray-900">{new Date(risk.createdAt).toLocaleDateString()}</dd>
                </div>
                <div>
                  <dt className="text-xs text-gray-500">Last Updated</dt>
                  <dd className="text-sm text-gray-900">{new Date(risk.updatedAt).toLocaleDateString()}</dd>
                </div>
                <div>
      ...
      <div className="max-w-7xl mx-auto py-8 px-4 sm:px-6 lg:px-8">
        <CommentsSection targetType="risk" targetId={riskId} />
      </div>
    </AppLayout>
  );
}
