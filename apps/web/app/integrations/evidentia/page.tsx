...
export default function EvidentiaGovernancePage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const settings = await api.get<{ evidentiaSyncEnabled: boolean }>(
        '/api/integrations/evidentia/settings'
      );
      ...
      if (st.data?.configured && st.data?.reachable) {
        const ext = await api.get<EvidentiaEvidence[]>(
          '/api/integrations/evidentia/external-evidence'
        );
      ...
      <div className="bg-white shadow">
        <div className="max-w-7xl mx-auto py-6 px-4 sm:px-6 lg:px-8">
          ...
          <p className="mt-2 text-gray-600 max-w-3xl">
            SAI remains the system of record for AI governance decisions. When configured, SAI pushes evidence
            lifecycle into Evidentia for audit-grade storage and review workflows.
          </p>
        </div>
      </div>
